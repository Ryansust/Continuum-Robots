...
% ========================================================================
%  Step 9.9: The Ultimate Comparison Plot (T-RO/TMECH Standard)
% =========================================================================
disp('   > Generating Final Three-Method Comparison Plot...');

figure('Name', 'Final Performance Comparison', 'Color', 'w', 'Position', [50, 50, 1400, 750]);
hold on; grid on;

sample_indices = 1:length(error_proposed);
c_proposed = [0.85, 0.33, 0.1]; % 橙红
c_brute    = [0.20, 0.60, 0.30]; % 森林绿
c_phys     = [0.4, 0.4, 0.4];    % 深灰

% 1. 绘制【纯物理模型】(误差最高，作为 Baseline)
plot(sample_indices, error_phys_final, '--', 'Color', c_phys, 'LineWidth', 1.5, ...
    'DisplayName', 'Physical Model (No Compensation)');

% 2. 绘制【暴力 MLP】(中等误差，作为 Ablation)
plot(sample_indices, error_brute_final, '-.', 'Color', c_brute, 'LineWidth', 2, ...
    'DisplayName', 'Vanilla MLP (End-to-End)');

% 3. 绘制【Proposed 方法】(误差最低)
fill([sample_indices, fliplr(sample_indices)], ...
     [movmean(error_proposed, 5)+0.5, fliplr(movmean(error_proposed, 5)-0.5)], ...
     c_proposed, 'FaceAlpha', 0.1, 'EdgeColor', 'none', 'HandleVisibility', 'off');
plot(sample_indices, error_proposed, '-o', 'Color', c_proposed, 'LineWidth', 3, ...
     'MarkerSize', 7, 'MarkerFaceColor', 'w', 'DisplayName', '\bf{Proposed (Cascaded Residual)}');

% 标注均值
yline(mean(error_phys_final), ':', 'Phys Mean', 'Color', c_phys, 'FontSize', 14, 'LineWidth', 1.2);
yline(mean(error_proposed), ':', 'Prop Mean', 'Color', c_proposed, 'FontSize', 14, 'LineWidth', 1.2, 'FontWeight', 'bold');

% 坐标轴与标签美化
set(gca, 'FontSize', 22, 'FontName', 'Times New Roman', 'LineWidth', 1.5, 'TickDir', 'out');
xlabel('Test Sample Index', 'FontSize', 24);
ylabel('Tip Pose Error [mm]', 'FontSize', 24);
title('Accuracy Benchmark: Physics vs. MLP vs. Proposed', 'FontSize', 26);
legend('Location', 'northeast', 'FontSize', 18, 'Interpreter', 'tex');

% 调整 Y 轴范围以适配物理模型的巨大误差
ylim([0, max([error_phys_final, error_brute_final])*1.2]);

disp('>>> All done! The ultimate performance comparison chart is ready.');
%% ========================================================================
...
for k = 1:3
    ...
    % 坐标轴修饰
    set(gca, 'FontSize', 22, 'FontName', standard_font, 'LineWidth', 1.5, 'TickDir', 'out', 'Box', 'off');
    xlabel('Test Sample Index', 'FontSize', 24, 'FontWeight', 'bold');
    ylabel(sprintf('Force %s [N]', comp_names{k}), 'FontSize', 24, 'FontWeight', 'bold');
    title(sprintf('Force Perception Fidelity: %s-Axis', comp_names{k}), 'FontSize', 26, 'FontWeight', 'bold');
    ...
    text(max(sample_indices)*0.05, min(gt_val) + 0.1*f_range, ...
        ...
        'FontSize', 18, 'FontName', standard_font, 'BackgroundColor', 'w', 'EdgeColor', 'k');
