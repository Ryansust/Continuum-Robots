...
%  Author:  Lin Yongxi (Optimized by AI Expert - Full Version)
% =========================================================================
clc; clear; close all;
...
%[红线修复 1]：建立 track_rows 数组，全生命周期追踪 Excel 原始行号 (从第3行开始读取)
track_rows_raw = (3:height(dataTable))'; 

% [红线修复 2]：单位转换，拉力必须乘以 0.00981 转换为 N
...
% 假设 Excel 中有 before 和 after 两个形态文本列，请根据你的 Excel 实际列号调整29和30
...
track_rows_sub = track_rows_raw(roi_mask); % 同步跟踪
...
for i = 1:N_sub
    % [核心逻辑约束] get_RealOffset_1S3CT 内部需保证执行了 R_mat * [0; -0.004; 0] 偏置补偿
    offset_b = get_RealOffset_1S3CT(pos_text_b_sub{i});
    offset_a = get_RealOffset_1S3CT(pos_text_a_sub{i});
    
    P_before_ideal(:, i) = reshape(offset_b(:, 3:end),[], 1); 
    P_after_sensor(:, i) = reshape(offset_a(:, 3:end),[], 1); 
...
% 1.5[红线修复 3] Data Cleaning: NaN & 10mm 几何畸变检测
...
for i = 1:N_sub
    ...
    for j = 2:6
        ...
        if dist_to_mid > 0.01 % 大于 10mm 跳变
            bad_idx(i) = true;
            break;
        end
    end
...
% =========================================================================
disp('--------------------------------------------------');
disp('2. Executing rotational augmentation...');[aug_F_diff, aug_F_after, aug_F_before, aug_Pb, aug_Pa, aug_gt_F, aug_hgt, aug_track_rows] = ...
    augment_data_by_rotation(F_diff, F_after, F_before, P_before_clean, P_after_clean, gt_F_clean, raw_hgt, track_rows_clean);

% [残差核心] 计算真实的 Delta P 作为 Target
aug_Delta_P = aug_Pa - aug_Pb;

%% ========================================================================
%  Step 3: Dataset Construction & Safety Check (原汁原味保留)
% =========================================================================
disp('--------------------------------------------------');
...
%% ========================================================================
%  Step 4: Net B - Force Estimation (原汁原味保留)
% =========================================================================
disp('--------------------------------------------------');
...
%% ========================================================================
%  Step 5: Net B - Location Sensing (Weighted Loss) (原汁原味保留)
% =========================================================================
disp('--------------------------------------------------');
...
%% ========================================================================
%  Step 6: Evaluation & Visualization (Net B) (原汁原味保留)
% =========================================================================
disp('--------------------------------------------------');
...
%% ========================================================================
%  Step 7: Net C - Shape Reconstruction (残差修复版)
% =========================================================================
disp('--------------------------------------------------');
...
feat_P_before = aug_Pb(:, v_mask); % P_before 仅作为特征输入

inputs_net_c =[feat_internal; feat_external; feat_location; feat_P_before];

% [红线修复 4]：Target 必须且只能是 Delta P
targets_net_c = aug_Delta_P(:, v_mask); 
track_rows_net_c = aug_track_rows(v_mask); % 同步跟踪行号
...
% [红线修复 5]：寻找最差案例，并开启独立 3D 物理大图
[~, sort_idx] = sort(tip_dist, 'descend');
worst_cases = sort_idx(1:min(7, length(sort_idx))); % 取最差的 3 个 Case

for k = 1:length(worst_cases)
    idx = worst_cases(k);
    orig_row = rows_test(idx); % 溯源到 Excel 行号
    
    P_p = [[0;0;0], reshape(pred_P_after(:, idx), 3, [])];
    P_r = [[0;0;0], reshape(real_P_after(:, idx), 3,[])];
    
    figure('Name', sprintf('Worst Case %d - Excel Row: %d', k, orig_row), 'Color', 'w', 'Position',[100, 100, 800, 800]);
    hold on; grid on; axis equal;
    
    % 绘制基座原点 (黄星) 和 RGB 基准坐标轴
    plot3(0,0,0, 'p', 'MarkerSize', 20, 'MarkerEdgeColor', 'k', 'MarkerFaceColor', 'y', 'DisplayName', 'Base Origin');
    quiver3(0,0,0, 0.05,0,0, 'r', 'LineWidth', 3, 'MaxHeadSize', 0.5, 'DisplayName', 'X-axis');
    quiver3(0,0,0, 0,0.05,0, 'g', 'LineWidth', 3, 'MaxHeadSize', 0.5, 'DisplayName', 'Y-axis');
    quiver3(0,0,0, 0,0,0.05, 'b', 'LineWidth', 3, 'MaxHeadSize', 0.5, 'DisplayName', 'Z-axis');
    
    % 绘制真值 (黑实线) 和 预测 (红虚线)
    plot3(P_r(1,:), P_r(2,:), P_r(3,:), 'k-o', 'LineWidth', 2.5, 'MarkerSize', 6, 'MarkerFaceColor', 'k', 'DisplayName', 'Ground Truth');
    plot3(P_p(1,:), P_p(2,:), P_p(3,:), 'r--^', 'LineWidth', 2, 'MarkerSize', 6, 'MarkerFaceColor', 'r', 'DisplayName', 'Prediction');
    
    % 绘制 Tip 误差向量 (紫粗线)
    plot3([P_r(1,end), P_p(1,end)],[P_r(2,end), P_p(2,end)],[P_r(3,end), P_p(3,end)], ...
          'm-', 'LineWidth', 4, 'DisplayName', sprintf('Tip Error: %.1fmm', tip_dist(idx)*1000));
    
    % [严格执行物理视角设置]
    set(gca, 'zdir', 'reverse', 'ydir', 'reverse');
    xlabel('X (m)'); ylabel('Y (m)'); zlabel('Z (m)');
    title(sprintf('Worst Case Evaluation | Excel Raw Row: %d', orig_row), 'FontSize', 14, 'Interpreter', 'none');
    legend('Location', 'best');
    view(30, 20);
end

%% ========================================================================
%  Step 9: Tip Error Analysis (原汁原味保留与合并)
% =========================================================================
disp('--------------------------------------------------');
...
%% ========================================================================
%  Helper Function: Data Augmentation (Fixed with Track Rows & P_after)
% =========================================================================
function[aug_Fd, aug_Fa, aug_Fb, aug_Pb, aug_Pa, aug_gF, aug_h, aug_tr] = ...
    ...
    gF_120 = R120 * gt_F;
    P_b_120 = reshape(R120 * reshape(P_before, 3,[]), 21, N);
    P_a_120 = reshape(R120 * reshape(P_after, 3,[]), 21, N);
    
    % 240度增强
    Fd_240 = F_diff(idx240, :); Fa_240 = F_after(idx240, :); Fb_240 = F_before(idx240, :);
    gF_240 = R240 * gt_F;
    P_b_240 = reshape(R240 * reshape(P_before, 3,[]), 21, N);
    P_a_240 = reshape(R240 * reshape(P_after, 3,[]), 21, N);
    ...
    aug_Pa =[P_after, P_a_120, P_a_240]; % 同步旋转 P_after
    aug_gF = [gt_F, gF_120, gF_240]; 
    aug_h  = [hgt, hgt, hgt];
    aug_tr =[track_rows; track_rows; track_rows]; % 复制追踪号
end