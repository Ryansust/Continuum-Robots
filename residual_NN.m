...
%  Module:  Pure Neural Network Baseline (Sensor-In, Sensor-Out)
%  Author:  Lin Yongxi (Logic Corrected & Restored)
% =========================================================================
clc; clear; close all;
...
% 🌟 提取传感器实测的 Before (特征) 和 After (目标) 位姿文本
% 假设 29 列是受力前原始位姿，38 列是受力后真实位姿
...
pos_text_before = pos_text_before_sub(~bad_idx); 
pos_text_after  = pos_text_after_sub(~bad_idx); 

F_diff = F_after - F_before;
N = length(raw_mag);
fprintf('   > Final effective samples: %d\n', N);

% 1.5 Kinematics Parsing (Both Before and After from SENSOR)
disp('   > Parsing sensor pose data (Before as Input, After as Target)...');
P_before_sensor = zeros(21, N); 
P_after_sensor  = zeros(21, N);
gt_F_vec = zeros(3, N);

for i = 1:N
    % Parse 3D coords for Before (Input Feature)
    r_off_b = get_RealOffset_1S3CT(pos_text_before{i});
    P_before_sensor(:, i) = reshape(r_off_b(:, 3:end), 21, 1); 
    
    % Parse 3D coords for After (Training Target)
    r_off_a = get_RealOffset_1S3CT(pos_text_after{i});
...
%% ========================================================================
%  Step 2: Data Augmentation (Synchronized Rotation)
% =========================================================================
disp('--------------------------------------------------');
disp('2. Executing synchronized rotational augmentation...');
% 使用修正后的增强函数，同时旋转 Before 和 After 位姿
...
raw_shape_tg = targets_shape_final(:, v_mask); % 用于 Net C
...
% Plotting
figure('Name', 'Net B: Location Results (Pure NN)', 'Color', 'w', 'Position', [100, 100, 1000, 400]);
subplot(1, 2, 1);
...
xlabel('Truth'); ylabel('Pred'); grid on;
...
%% ========================================================================
%  Step 7: Net C - Shape Reconstruction (Pure NN Baseline)
% =========================================================================
disp('--------------------------------------------------');
disp('7. Training Net C (Shape Reconstruction - Pure NN Baseline)...');

% 7.1 Construct Inputs (Consistent with the logic: Sensor In -> Sensor Out)
feat_internal = raw_in(7:12, :);      % F_after
feat_external = targets_f_final(:, v_mask); % F_ext (GT)
feat_location = raw_tg;               % Location (GT)

inputs_net_c = [feat_internal; feat_external; feat_location];
targets_net_c = raw_shape_tg; % 🌟 真实碰撞后的位姿
...
% 3D Visualization
figure('Name', '3D Shape Reconstruction (Pure NN)', 'Color', 'w', 'Position', [100, 100, 1200, 500]);
num_plot = 4;
...
for k = 1:num_plot
    ...
    if k==1, legend('Actual After', 'Pred After'); end
    view(30, 20);
end
...
% Visualization
figure('Name', 'Tip Error Analysis (Pure NN)', 'Color', 'w', 'Position', [100, 200, 1000, 400]);
...
xlabel('X'); ylabel('Y'); zlabel('Z'); title('Tip Tracking (Pure NN)'); legend([h1, h2], {'GT', 'Pred'}); view(45, 30);

subplot(1, 2, 2);
histogram(tip_dist * 1000, 30, 'FaceColor', [0.3 0.3 0.3]);
xline(tip_mae * 1000, 'r--', 'LineWidth', 2);
xlabel('Error (mm)'); ylabel('Count'); title('Tip Error Dist.'); grid on;

disp('>>> All done. Sensor-based Pure NN Baseline training complete.');

%% === Save Baseline Model ===
save('Pure_NN_Baseline_Model.mat', 'net_force', 'net_loc', 'net_shape', 'ps_in_c', 'ps_out_c', 'test_idx');

%% ========================================================================
%  Helper Functions: Consistent Data Augmentation
% =========================================================================
function [aug_Fd, aug_Fa, aug_Fb, aug_Pb, aug_Pa, aug_gF, aug_h] = ...
    ...
    % Synchronized rotation for both Before and After poses
