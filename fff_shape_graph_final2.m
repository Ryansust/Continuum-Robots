...
disp('      Compact axis scaling is used to make the robot shape larger.');
disp('      No subplot or tiledlayout is used.');

export_this_figure = true;
output_folder = 'IEEE_6_right_half_panels_mm_compact2';
...
% For checking coordinate labels:
...
origin_axis_len = 30;       % mm
origin_axis_lw  = 5;

plot_box_aspect = [1 1 2.1];   % make vertical shape visually larger
...
for col_tmp = 1:3
    ...
    F0_tmp = local_get_disk_position(Pg_tmp,contact_disk);
    local_contact_tmp = local_get_disk_position(Pg_tmp,4);
...
%   4) make the robot occupy more vertical space
...
while true
    ...
    x_lim_global = [x_start, x_start + total_x];

    if x_lim_global(1) <= x_raw(1) && x_lim_global(2) >= x_raw(2)
        break;
    end
...
x_ticks_global = x_lim_global(1):segment_x:x_lim_global(2);
...
while true
    ...
    if y_lim_global(1) <= y_raw(1) && y_lim_global(2) >= y_raw(2)
        break;
    end
...
for col = 1:3
    ...
    for row = 1:2
        ...
        if show_prior_context_for_nonprior
            ...
            scatter3(ax, P0(1,:), P0(2,:), P0(3,:), 200, ...
                'Marker', 'o', ...
        ...
        scatter3(ax, Pg(1,:), Pg(2,:), Pg(3,:), 210, ...
            'Marker', 'o', ...
            'MarkerFaceColor', c_gt_marker, ...
            'MarkerEdgeColor', 'k', ...
            'LineWidth', 2);

        scatter3(ax, Pc(1,:), Pc(2,:), Pc(3,:), 210, ...
            'Marker', marker_style, ...
            'MarkerFaceColor', c_cmp_marker_fill, ...
            'MarkerEdgeColor', c_cmp, ...
            'LineWidth', 2);

        scatter3(ax, Pg(1,end), Pg(2,end), Pg(3,end), 226, ...
            'Marker', 'o', ...
            'MarkerFaceColor', c_gt, ...
            'MarkerEdgeColor', 'k', ...
            'LineWidth', 2);

        scatter3(ax, Pc(1,end), Pc(2,end), Pc(3,end), 226, ...
            'Marker', 'o', ...
        ...
        contact_disk = 4;          % Your experiment uses Disk4

        contact_pt = local_get_disk_position(Pg, contact_disk);
        ...
        if show_origin_frame
            quiver3(ax, 0, 0, 0, origin_axis_len, 0, 0, ...
                0, 'Color', 'r', 'LineWidth', origin_axis_lw, 'MaxHeadSize', 2);

            quiver3(ax, 0, 0, 0, 0, origin_axis_len, 0, ...
                0, 'Color', 'g', 'LineWidth', origin_axis_lw, 'MaxHeadSize', 2);

            quiver3(ax, 0, 0, 0, 0, 0, origin_axis_len, ...
                0, 'Color', 'b', 'LineWidth', origin_axis_lw, 'MaxHeadSize', 2);
        end
