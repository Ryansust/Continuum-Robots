...
% 1.3 ROI Filtering (Keep Nodes 3o m, 4, 5)
...
% 1.5 [核心改变] 使用物理模型生成理论 $P_{before}$ 代替 Nokov 原始值
disp('   > 正在调用物理模型生成理论 $P_{before}$ (作为理想训练目标)...');

% ---- 初始化物理模型参数 (使用你之前调好的参数) ----
...
for i = 1:N
    % A. 提取该样本真实的碰撞前拉力 (来自 Excel 11-16 列，已存入 F_before)
    F_tendon_raw = F_before(:, i) * 0.00981; 
    % 应用你发现的拉力映射顺序 [5,6,1,2,3,4]
    F_tendon_sim = [F_tendon_raw(5); F_tendon_raw(6); F_tendon_raw(1); F_tendon_raw(2); F_tendon_raw(3); F_tendon_raw(4)];
    
    % B. 调用物理模型计算零外力状态 (F_ex = [0;0;0])
    % touch_id 设为末端 14 即可
    [P_Theo, ~, R_mat, ~, ~, ~] = solve_continuum_shape_nofig(...
        tendon_p, section_p, D_p, E_p, L_ap, L_bp, N_dp, H_listp, mu_p, delta_alphap, G_loadp, [0;0;0], F_tendon_sim, 14);
    
    % C. 计算 Marker 的 4mm 表面偏置 (为了保持与传感器观测一致性)
    offset_dist = 0.004; 
    V_local = [0; -offset_dist; 0]; 
    P_m = zeros(3, size(P_Theo, 2));
    for pt = 1:size(P_Theo, 2)
        P_m(:, pt) = P_Theo(:, pt) + R_mat(:, :, pt) * V_local;
    end
    
    % D. 精准提取 7 个躯干 Marker 对应的圆盘索引 (id_list = [2,4,6,8,10,12,14])
    id_list = [2, 4, 6, 8, 10, 12, 14]; 
    kappa = (size(P_Theo, 2) - 1) / 14; 
    marker_idx = round(id_list * kappa) + 1;
    
    % 存入 P_before (此时的 P_before 是经过物理校正的“纯净”形态)
    P_before(:, i) = reshape(P_m(:, marker_idx), 21, 1); 
...
fprintf('   > 物理校正后的 P_before 生成完毕，共有 %d 组样本。\n', N);
...
% disp('Successfully saved as Force2Position_Model.mat');
%% ========================================================================
