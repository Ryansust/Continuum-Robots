...
disp('      Every figure keeps markers, force arrow, contact node, tip error line, and origin frame.');
disp('      Unified grid, no text, thicker lines.');

export_this_figure = true;
output_folder = 'IEEE_6_right_half_panels';
...
c_gt              = [0, 0, 0] / 255;
c_prior_context   = [160, 160, 160] / 255;
...
x_lim_global = [min(global_all_pts(1,:))-global_pad, max(global_all_pts(1,:))+global_pad];
y_lim_global = [min(global_all_pts(2,:))-global_pad, max(global_all_pts(2,:))+global_pad];
z_lim_global = [min(global_all_pts(3,:))-global_pad, max(global_all_pts(3,:))+global_pad];

x_ticks_global = linspace(x_lim_global(1), x_lim_global(2), 4);
y_ticks_global = linspace(y_lim_global(1), y_lim_global(2), 4);
z_ticks_global = linspace(z_lim_global(1), z_lim_global(2), 4);
...
disp('No title, no labels, no tick text, no legend, no X/Y/Z text.');
disp('==========================================================================');
disp(' ');
...
for col = 1:3
    ...
    for row = 1:2
        ...
        tip_err_mm = norm(Pc(:, end) - Pg(:, end)) * 1000;
        shape_err_mm = mean(vecnorm(Pc - Pg, 2, 1)) * 1000;
        ...
        if show_axis_labels
            xlabel(ax, 'X [m]', 'FontName', font_name, 'FontSize', 14, 'FontWeight', 'bold');
            ylabel(ax, 'Y [m]', 'FontName', font_name, 'FontSize', 14, 'FontWeight', 'bold');
            zlabel(ax, 'Z [m]', 'FontName', font_name, 'FontSize', 14, 'FontWeight', 'bold');
        else
            xlabel(ax, '');
            ylabel(ax, '');
            zlabel(ax, '');
