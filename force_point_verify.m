...
condition = (force_array(:, 3) ~= 0) & all(force_array(:, 4:6) == 0, 2);
%condition = (force_array(:, 1) == -20) & all(force_array(:, 2:6) == -20, 2);
...
for k = 1:num_plots
    ...
    F_ex = f_mag * u_vec;
    switch dir_code
        case 2, u_vec_sim =[1; 0; 0];
        case 3, u_vec_sim =[sind(45); cosd(45); 0];
        case 4, u_vec_sim =[0; 1; 0];    
    end
    F_ex_sim = f_mag * u_vec_sim;
    ...
    [P_Theo, ~, ~, ~, ~, ~] = solve_continuum_shape_nofig(tendon, section, D, E, L_a, L_b, N_d, H_list, mu, delta_alpha, G_load, F_ex_sim, F_tendon2, touch_id);
    %误差计算
    Tip_Real = P_Real(:, end); 
    Tip_Theo = P_Theo(:, end);
