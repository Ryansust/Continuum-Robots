...
if __name__ == "__main__":
    ...
    def generate_adjacent_tensions():
        ...
        for idx in active_indices:
            ...
            if idx in LOW_IMPACT_AXES:
                # 小变化线 - 施加较大力值
                tension = random.uniform(MAX_TENSION * 0.7, MAX_TENSION)
            else:
                # 大变化线 - 施加较小力值
    ...
    if start==1:
        ...
        disturbance_encoders = None  # 扰动后的编码器值
        disturbance_forces= None#扰动后的力值
        disturbance_detected = False  # 扰动检测标志
        force_threshold = 30  # 力变化阈值
        stability_count = 0  # 稳定计数器
        stability_threshold = 10  # 稳定所需连续周期数

        # 主控制循环
        for i in range(200):  # 增加总循环次数
            # 读取力传感器数据
            ...
            # 扰动检测逻辑 (100次循环后)
            if i > 100 and not disturbance_detected and baseline_forces is not None:
                # 检查是否有力值超过阈值
                for idx in range(6):
                    force_delta = abs(current_forces[idx] - baseline_forces[idx])
                    if force_delta > force_threshold:
                        print(f"\n检测到力突变! 传感器 {idx} 变化: {force_delta:.1f}N")
            ...
            # 稳定性监测 (检测到扰动后)
            if disturbance_detected and baseline_forces is not None:
                # 检查力值是否已稳定
                force_stable = True
                for idx in range(6):
                    if abs(current_forces[idx] - baseline_forces[idx]) > force_threshold / 2:
                        force_stable = False
                        break

                if force_stable:
                    stability_count += 1
                    print(f"稳定性计数: {stability_count}/{stability_threshold}")
                else:
                    stability_count = 0

                # 如果稳定计数达到阈值
                if stability_count >= stability_threshold:
                    disturbance_encoders = current_encoders.copy()
                    print("\n系统已重新稳定!")
                    print("扰动后编码器值: ", end="")
                    print(" | ".join(f"{e:>8.2f}" for e in current_encoders))
                    #记录扰动后的力值和编码器值
                    disturbance_forces = current_forces.copy()
                    disturbance_encoders=current_encoders.copy()
                    # 标记扰动处理完成
            ...
            # 如果已记录扰动后数据，可以选择提前结束
            if disturbance_encoders is not None and i > 120:
                ...
                print("扰动后力值：", end="")
                print(", ".join(f"{e:>8.2f}" for e in disturbance_forces))
                print("扰动后编码器值：", end="")
                print(", ".join(f"{e:>8.2f}" for e in disturbance_encoders))
                # 创建单行CSV格式的输出
                csv_line = [
                    # 目标力值
                    ...
                    # 扰动后力值
                    f"{disturbance_forces[0]:.2f}", f"{disturbance_forces[1]:.2f}", f"{disturbance_forces[2]:.2f}",
                    f"{disturbance_forces[3]:.2f}", f"{disturbance_forces[4]:.2f}", f"{disturbance_forces[5]:.2f}",
                    # 扰动后编码器
                    f"{disturbance_encoders[0]:.2f}", f"{disturbance_encoders[1]:.2f}",
                    f"{disturbance_encoders[2]:.2f}",
                    f"{disturbance_encoders[3]:.2f}", f"{disturbance_encoders[4]:.2f}",
                    f"{disturbance_encoders[5]:.2f}"
                ]
